...
/*
01. **Idle**: ポインタが何も操作していない状態。
02. **Down**: 要素に指が触れている状態。2つ目の指が触れた場合は**DoubleDown**に遷移する。一定時間が経過する前に離れた場合は**FirstTap**に遷移する。一定時間が経過すると、権限がある場合はVibration APIを用いてバイブレーションを発生させたのち、**Hold**に遷移する。また、指が閾値を超える移動をした場合は**Drag**に遷移する。
03. **Drag**: この状態にある場合、指の移動に合わせてポインタを移動する。指が離れた場合は**Idle**に遷移する。
04. **Hold**: 指が離れたときに**FirstTap**に遷移する。
05. **FirstTap**: 指が離れてから一定時間が経過するまでの間に再度指が触れた場合、**スライドを移動**する。**Hold**を経由していた場合は前のページに、**Down**からこの状態になった場合は次のページに移動する。そのあと、**PageMoved**に遷移する。タップされなかった場合は**Idle**に遷移する。
06. **PageMoved**: 指が離れると**Idle**に遷移する。
07. **DoubleDown**: 2つ目の指が触れた状態。いずれかの指が離れた場合は**Wait**に遷移する。2つの指が閾値以上の移動を発生させた時、違う方向に移動し始めた場合は**Pinch**に遷移する。同じ方向に移動し始めた場合は**Pan**に遷移する。
08. **Pinch**: この状態にある場合、2つの指の距離に応じてスライドを拡大縮小する。どちらかの指が離れた場合は**Wait**に遷移する。
09. **Pan**: この状態にある場合、2つの指の移動に応じてスライドを移動する。どちらかの指が離れた場合は**Wait**に遷移する。
10. **Wait**: 全ての指が離れたら**Idle**に遷移する。

`pointercancel`イベントが発生した場合は、どの状態にあっても**Wait**に遷移する。
*/
...
export function usePointerControl({
  sessionId,
  fullScaleAdjustment,
  pdfImageSize,
  onSlide,
  ...
  function handlePointerDown(pointerId: number, point: Point) {
    ...
    if (stateRef.current === 'Idle' && activePointersRef.current.size === 1) {
  ...
  function handlePointerEnd(pointerId: number) {
    ...
    if (endingState === 'PageMoved') {
      transitionTo(activePointersRef.current.size === 0 ? 'Idle' : 'Wait');
      return;
    }
  ...
  const onPointerLeave: PointerEventHandler<HTMLDivElement> = event => {
    if (event.buttons === 0 && activePointersRef.current.has(event.pointerId)) {
      handlePointerEnd(event.pointerId);
    }
  };
